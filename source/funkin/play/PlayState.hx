	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			case 'Hey!':
             ...
             case 'DodgeEvent':
      	if (ClientPrefs.data.mechanicsMode == 'OFF')
		  return;

	     var dodgeTime:Float = Std.parseFloat(value1);
     	if (Math.isNaN(dodgeTime) || dodgeTime <= 0)
	  	dodgeTime = 1.5;
	      ...
	      if (dodgeTimer != null)
  	{
     	...
     	dodgeTimer = new FlxTimer().start(dodgeTime, function(t:FlxTimer)
  	{
	  	 if (canDodge && !dodged)
	  	{
		  	if (ClientPrefs.data.mechanicsMode == 'HELL')
			  	health = 0;
			  else
			  	health -= 1; // AMADOR
